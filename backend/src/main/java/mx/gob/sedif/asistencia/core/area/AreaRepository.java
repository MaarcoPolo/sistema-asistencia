...
public interface AreaRepository extends JpaRepository<Area, Integer> {
    ...
    @Query("SELECT a FROM Area a WHERE LOWER(a.nombre) LIKE LOWER(concat('%', :key, '%')) AND a.id IN :areaIds AND a.estatus <> mx.gob.sedif.asistencia.util.enums.Estado.DELETED")
    Page<Area> findByNombreAndIds(@Param("key") String key, @Param("areaIds") Set<Integer> areaIds, Pageable pageable);

    List<Area> findByAreaPadreInAndEstatus(Set<Area> areasPadre, Estado estatus);
}